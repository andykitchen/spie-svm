...
def extract_patch(image, position, patch_pixels=128, patch_mm=60, layers=3, layer_spacing_mm=3., augment=False, sigma=2.0, sigma_shift=0.0, sigma_theta=0.1):
    ...
    cc = c - 0.5*np.dot(v, b) + sigma_shift*np.random.randn(3)
...
def load_patient_images(findings_path, doi_path, ktrans_path, tqdm=tqdm):
    ...
    def process_row(row):
        ...
        return (patient_id, pos, clin_sig, zone, paths)

    it = tqdm(findings_df.iterrows(), total=len(findings_df))
    patient_paths = [process_row(row) for ix, row in it]

    def read_images(paths):
        return [read_image_with_format(path) for path in paths]

    it = tqdm(patient_paths)
    patient_images = [(patient_id, pos, clin_sig, zone, read_images(paths))
                      for patient_id, pos, clin_sig, zone, paths in it]
